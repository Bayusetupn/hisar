...
const ProfileOverview = () => {
  ...
  const [dataAgen, setdataAgen] = useState({id: "",bergabung: "",foto: "",no_ktp: "",nama: "", alamat: "", no_telepon: "", username: ""})
  ...
  const agen = async() =>{
    try {
      await api.get(`/agen/${location.state.id}`, {withCredentials: true}).then((res)=>{
        setdataAgen({id: res.data.data.id,bergabung: res.data.data.dibuat_pada,foto: res.data.data.foto,no_ktp: res.data.data.no_ktp,nama: res.data.data.nama, alamat: res.data.data.alamat, no_telepon: res.data.data.no_telepon, username: res.data.data.username})
      })
      .catch(err=>{
        console.log(err)
      })
    ...
    <div className="flex w-full flex-col h-screen overflow-auto gap-5 p-5 dark:!bg-navy-900">
      ...
      <div className="w-ful mt-3 flex h-fit flex-col gap-5 lg:grid lg:grid-cols-12">
        <div className="col-span-3 lg:!mb-0">
          <Banner historyData={historyData} ktp={dataAgen.no_ktp} gabung={dataAgen.bergabung} foto={dataAgen.foto} nama={dataAgen.nama} alamat={dataAgen.alamat} no={dataAgen.no_telepon} username={dataAgen.username}/>
        </div>
